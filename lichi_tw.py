...
class LIChI(nn.Module):
    ...
    def block_matching(input_x, k, p, w, s):
        ...
        #((H - p + 1) // -s) H= 전체 이미지 크기, p= 패치 크기 +1= 시작 인덱스 s= 스트라이드 => 총 몇개의 패치가 있는지
        ...
        ind_H_ref = torch.arange(0, H-p+1, step=s, device=input_x.device) #=> 패치의 시작점 
    ...
    def forward(self, y, sigma=25.0, constraints='affine', method='n2n',
        ...
        # print(torch.std(z - x, dim=(2,3), keepdim=True, unbiased=False).item(),sigma)
        # plt.imshow(x[0][0],cmap='gray')
        # plt.axis('off')
        # plt.show()
        write_png(torch.clamp(x[0][0].unsqueeze(0),0,255).byte(), "LICHIdemo/SET12_sigma50_addnoise/InitialPilot.png")
        for m in range(self.M):
            
            if sigma <1:
                pass
            else:
                # sigma = torch.std(z - x, dim=(2,3), keepdim=True, unbiased=False).item()
                tau = (1-(m+1)/self.M)*0.75
                # x = x + 5 * torch.randn_like(x)
                # z = z + 5 * torch.randn_like(z)
                # sigma = torch.std(z - x, dim=(2,3), keepdim=True, unbiased=False).item()
                if m%3==0:
                    x, z, indices = self.step2(z, x, y, sigma, tau)
                    write_png(torch.clamp(x[0][0].unsqueeze(0),0,255).byte(), f"LICHIdemo/SET12_sigma50_addnoise/x{m}.png")
                    write_png(torch.clamp(x[0][0].unsqueeze(0),0,255).byte(), f"LICHIdemo/SET12_sigma50_addnoise/z{m}.png")
                    # z, x  = y, self.step1(y, sigma)
                else:
